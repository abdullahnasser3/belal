...
final class WPML_Meta_ID_Mapper_UI {
    ...
    public function __construct() {
        ...
        add_filter('wpml_sync_custom_field_copied_value', [$this, 'filter_wpml_copied_value'], 50, 5);
    ...
    public function register_admin_page() {
        ...
        add_options_page(
            'WPML Repeater Taxonomy Mapper',
            'WPML Repeater Taxonomy',
            'manage_options',
            'wpml-repeater-taxonomy-mapper',
    ...
    public function render_admin_page() {
        ...
        <div class="wrap">
            ...
            <p>
                Raw value copy: <a href="<?php echo esc_url(admin_url('options-general.php?page=wpml-raw-copy-sync')); ?>">WPML Raw Copy Sync</a>.
                Repeater fields (taxonomy IDs): <a href="<?php echo esc_url(admin_url('options-general.php?page=wpml-repeater-taxonomy-mapper')); ?>">WPML Repeater Taxonomy</a>.
            </p>
    ...
    public function render_repeater_page() {
        ...
        ?>
        <div class="wrap">
            <h1>WPML Repeater Taxonomy Mapper</h1>
            <p>Map taxonomy term IDs inside repeater sub-fields (e.g. <code>cruise_runs_day</code>, <code>cruise_runs_origin</code>) to WPML translated IDs. Use when your repeater stores taxonomy IDs in separate meta keys.</p>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="widefat striped" id="wpml-repeater-rules-table">
                    ...
                    <tr>
                        ...
                        <th>Taxonomy (object type)</th>
                        <th>Remove</th>
                    </tr>
                    </thead>
                    <tbody>
                    <?php if (empty($rules)): ?>
                        <tr class="no-items"><td colspan="5">No repeater rules yet. Add rows: one row per sub-field (e.g. cruise_runs_day + taxonomy for day).</td></tr>
                    <?php else: ?>
                        <?php foreach ($rules as $idx => $rule): ?>
                            <?php
                            ...
                            <tr>
                                ...
                                <td><input type="text" class="regular-text" name="repeater_rules[<?php echo (int) $idx; ?>][sub_fields][<?php echo (int) $sf_idx; ?>][taxonomy]" value="<?php echo esc_attr($sub['taxonomy']); ?>" /></td>
            ...
            <hr />
            <h2>Bulk Sync</h2>
            <p>Run repeater taxonomy mapping on all posts that match the rules above.</p>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
            ...
            (function () {
                ...
                addBtn.addEventListener('click', function () {
                    ...
                    tr.innerHTML = `
                        ...
                        <td><input type="text" class="regular-text" name="repeater_rules[${gIdx}][sub_fields][${sIdx}][taxonomy]" value="" /></td>
                        <td><button type="button" class="button-link-delete wpml-repeater-remove-row">Remove</button></td>
                    `;
                    tableBody.appendChild(tr);
                });
    ...
    public function handle_save_repeater_settings() {
        ...
        foreach ($raw as $rule) {
            ...
            foreach ($sub_fields_raw as $sub) {
                ...
                $taxonomy = isset($sub['taxonomy']) ? sanitize_key((string) $sub['taxonomy']) : '';
                if ($meta_key !== '' && $taxonomy !== '') {
                    $sub_fields[] = ['meta_key' => $meta_key, 'taxonomy' => $taxonomy];
                }
            }
            if (empty($sub_fields)) {
                continue;
        ...
        wp_safe_redirect(add_query_arg(['page' => 'wpml-repeater-taxonomy-mapper', 'repeater_updated' => '1'], admin_url('options-general.php')));
        exit;
    }
    ...
    public function handle_bulk_sync_repeater() {
                ...
                do {
                    ...
                    if (!empty($q->posts)) {
                        foreach ($q->posts as $post_id) {
                            ...
                            if (empty($lang_info['language_code'])) continue;
                            
                            // For repeater, we want to process translations even if they don't have source_language_code set
                            // (e.g., imported posts that haven't been properly linked yet)
                            // So we check if it's a translation by looking for original post
                            $is_translation = false;
                            if (!empty($lang_info['source_language_code'])) {
                                $is_translation = true;
                            } else {
                                // Check if there's an original post in WPML
                            ...
                            // Skip source posts only if translations_only is enabled AND this is not a translation
                            $translations_only = (int) get_option(self::OPTION_TRANSLATIONS_ONLY, 1) === 1;
                            ...
                            $scanned++;
                            if ($this->apply_repeater_rules_for_post($post_id, $lang_info['language_code'], !empty($lang_info['source_language_code']) ? $lang_info['source_language_code'] : null)) {
                                $changed++;
                            }
                        }
                    }
        ...
        wp_safe_redirect(add_query_arg([
            'page' => 'wpml-repeater-taxonomy-mapper',
                ...
                do {
                    ...
                    if (!empty($q->posts)) {
                        foreach ($q->posts as $post_id) {
                            ...
                            if ($this->should_skip_for_source_post($lang_info, $post_id, $post_type)) {
                                continue;
                            }
                            ...
                            $did_change = $this->map_rules_for_post(
                                ...
                                !empty($lang_info['source_language_code']) ? $lang_info['source_language_code'] : null
                            );
                            if ($did_change) {
                                $changed++;
                            }
    ...
    public function filter_wpml_copied_value($copied_value, $post_id_from, $post_id_to, $meta_key, $args) {
        if (!defined('ICL_SITEPRESS_VERSION')) {
            return $copied_value;
        }

        $post_id_to = (int) $post_id_to;
        if ($post_id_to <= 0 || !is_string($meta_key) || $meta_key === '') {
            return $copied_value;
        }

        $target_post = get_post($post_id_to);
        ...
        $lang_info = $this->get_lang_info($post_id_to);
        if (empty($lang_info['language_code'])) {
            return $copied_value;
        }
        if ($this->should_skip_for_source_post($lang_info, $post_id_to, $target_post->post_type)) {
            return $copied_value;
        }

        $source_lang_info = $this->get_lang_info((int) $post_id_from);
    ...
    private function get_repeater_rules(): array {
        ...
        foreach ($rules as $rule) {
            ...
            foreach ($rule['sub_fields'] as $sub) {
                if (!is_array($sub) || empty($sub['meta_key']) || empty($sub['taxonomy'])) {
                    continue;
                }
                $sub_fields[] = [
                    'meta_key' => sanitize_text_field((string) $sub['meta_key']),
                    'taxonomy' => sanitize_key((string) $sub['taxonomy']),
                ];
            }
            if (empty($sub_fields)) {
                continue;
    ...
    private function map_single_value_by_rule($value, $type, $object_type, $target_lang, $source_lang = null, $storage_mode = 'auto') {
        ...
        if ($is_numeric) {
            ...
            // Fallback: some fields store term_taxonomy_id instead of term_id.
            if ($type === 'term') {
